...
description = "Zig on the STM32F4 Discovery"
slug = ""
authors = []
...
keyboards background. I liked the premise of Zig's aim to be a "better C", opposed to other
...
This post will walk through the example, discussing some of the choices I've made and tools I used.
You can find the final source code [here](https://github.com/rbino/zig-stm32-blink), I suggest you
go through it while reading the post. I won't provide super-detailed steps to build the example from
the ground up, but you can look at `git log` to follow all my attempts (including failures).
...
the STM32F407VG has an hardware Floating Point Unit and we will enabled it in the system
...
For my example I added the bare minimum to make the CPU happy: I implemented a `resetHandler`, while
pointing to (overridable) empty handlers for the other exceptions and ignoring all other interrupts.

It's possible to see the structure of the STM32F407 vectore table in Section 2.3.4 of the [STM32
...
pointer, after that there are 15 32-bit words representing pointers to the handlers (with some
...
All handlers are `extern` and the linker script exports weakly linked symbols which point to either
...
initialization steps which are needed before proceeding to execute the `main` function.

The first step is initializing the `.data` section. This section contains global variables that are
initialied with a specific value. For example, if in our code we have a global variable like
...
initialization code is going to be responsible of copying the initial value to the RAM to initialize
the variable.
...
At this point we have the bare-minimum to run arbitrary Zig code. The only thing missing is a way to
access memory-mapped peripherals. We could do what is usually done in C: search for addresses in the
datasheet (or use a vendor-provided set of `#defines`), access them as as `u32` values numbers and
perform bitwise operations using flags and masks, but is there a better way?
...
source file which allows to access registers using packed structs. The generated code is the one
contained in `registers.zig`.

Armed with an handy way to access registers, we can do some more system initialization and finally
blink some LEDs.
...
an handy tool to generate all the values needed to initialize all clock domains without having to
...
Integrating a custom command as build step is really easy. After installing the tool, just run `zig
build flash`. This will produce a raw binary from the ELF, which is needed by `st-flash`, and then
it will flash it using `st-flash`.
...
Let's start with saying that I think that Zig is a cool language. It's simple enough that I felt confident
tackling the issues with the `svd2zig` tool after less than a week after I was using it. The
documentation is still a little lacking, especially for the `std` libraray, but this is compensated
by the fact that you can actually read the `std` library source code and understand what it's doing.
...
right balance for these kind of devices. I tried learning some Rust last year and while I
appreciated some of its insights, I didn't enjoy the experience of trying it on an embedded device
like I did with Zig.
